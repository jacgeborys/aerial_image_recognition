...
class GPUMonitor:
    ...
    def start(self):
        self.running = True
        # Create an initial line for GPU stats to anchor its position
        print("\nGPU: Initializing...", end='', flush=True)
    ...
    def _monitor_loop(self):
        while self.running:
            try:
                ...
                # Move up one line, clear it, print status, then move back down
                print(f"\033[A\r{status}\033[K\n", end='', flush=True)
                ...
                print(f"\033[A\rMonitoring error: {str(e)}\033[K\n", end='', flush=True)
